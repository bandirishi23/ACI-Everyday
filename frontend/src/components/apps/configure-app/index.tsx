...
export function ConfigureApp({
    ...
    try {
      ...
      switch (submitter.name) {
        case FORM_SUBMIT_COPY_OAUTH2_LINK_URL:
          await copyOAuth2LinkURL(name);
          break;
        case FORM_SUBMIT_LINK_OAUTH2_ACCOUNT:
          await linkOauth2Account(name);
          break;
  ...
  const fetchOAuth2LinkURL = async (
    ...
    if (afterOAuth2LinkRedirectURL === undefined) {
      return await getOauth2LinkURL(appName, apiKey);
    } else {
  ...
  const copyOAuth2LinkURL = async (
    ...
    try {
      const url = await fetchOAuth2LinkURL(appName);
  ...
  const linkOauth2Account = async (
    ...
    try {
      ...
      // window.location.href = oauth2LinkURL;  // This line is removed
    } catch (error) {
  ...
  const linkNoAuthAccount = async (
    ...
    try {
      await createNoAuthLinkedAccount(appName, apiKey);
      toast.success("account linked successfully");
    } catch (error) {
