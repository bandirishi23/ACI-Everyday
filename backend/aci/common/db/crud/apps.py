...
def search_apps(
    ...
    logger.info(
        ...
        extra={
            "filters": {
                ...
                "has_intent": intent_embedding is not None,
                "limit": limit,
                "offset": offset,
            }
        }
    ...
    if public_only:
        statement = statement.filter(App.visibility == Visibility.PUBLIC)
        logger.debug("Applied public visibility filter")

    # Filter out inactive apps
    if active_only:
        statement = statement.filter(App.active)
        logger.debug("Applied active status filter")

    # Filter by app names
    if app_names is not None:
        statement = statement.filter(App.name.in_(app_names))
        logger.debug(f"Filtered by app names: {app_names}")

    # Filter by categories
    if categories is not None:
        statement = statement.filter(App.categories.overlap(categories))
        logger.debug(f"Filtered by categories: {categories}")
    ...
    if intent_embedding is not None:
        ...
        logger.debug("Applied vector similarity sorting")
    ...
    try:
        ...
        logger.info(
            ...
            extra={
                ...
                "has_similarity_scores": intent_embedding is not None
            }
        )

        # Return results with similarity scores if intent was provided
        if intent_embedding is not None:
